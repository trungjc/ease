...
$(document).ready(function(){
	$(".order_details tfoot tr:first").find("small").remove();
	$(".order_details tfoot tr:last td").append("<small>(Chưa bao gồm thuế VAT)</small>");
});
</script>
...
<?php endif; ?>
		...
		<address>
			...
			echo '<br />';	
				if ( $order->billing_email ) {
				echo __( 'Email:', 'woocommerce' ) . $order->billing_email .'<br />';
			}
			
			if ( $order->billing_phone ) {
				echo __( 'Telephone:', 'woocommerce' ) . $order->billing_phone . '<br />';
			}
			?>
		</address>
