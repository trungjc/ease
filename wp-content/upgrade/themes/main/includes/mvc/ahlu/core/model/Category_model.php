		 ...
		 public function toList($limit=10,$page=1,$isPaged=false){
			...
			$query = "Select post.*,metap.meta from  ( SELECT group_concat(meta) as meta,mp.ID  from (SELECT
			   ...
			   ELSE CONCAT(meta_key, '=', meta_value)
                ...
                ) as mp GROUP BY ID )  as metap, ( select post.* from {$this->db->posts} post join {$this->db->term_relationships} t on t.object_id=post.ID where t.term_taxonomy_id={$this->category->term_id}) as post where metap.ID= post.ID and post.post_status='{$this->post_status}' order by post.post_date desc";
...
///////////////////////////////////////////////////////////////////////////	   
       ...
       * @param mixed $formats
       */
       public function menu($formats=null,$selected = null,$f=null){
           $s = "";
           $def = array(
            "class"=> "nav",
            "classUL"=>""
