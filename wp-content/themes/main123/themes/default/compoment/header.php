	...
	jQuery(document).ready(function($) {
		...
		$(document).bind("mini-cart-update",function(e,data){
			...
			if(list.find("li.empty").length!=0){
				$(".cart-holder a.total_pro").find("span").html(0);
			}else{
				$(".cart-holder a.total_pro").find("span").html(list.find("li").length);
			}
		});
								...
								$('.add_to_cart_button').on( 'click', function(e) {
									...
									if($(".widget_shopping_cart_content").find(".pro-"+$this.attr("data-product_id")).length!=0){
										...
										$.ajax({
								            ...
								            data: {"action": "add_service_enquiry",id_product:$(this).attr("data-product_id"),type:type},
											async: false, //wait
								            success: function(data){
												if(data==1){
												}else{
										...
										//show message ok
										if(ok){
											var msg = 'Your item '+type+' enquiry "'+$this.attr("data-title")+'" added successfully.';
